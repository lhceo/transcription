...
1. Railway → `transcription` サービス → **Usage** または **Metrics** タブ
2. 「Volume Usage」が現在のストレージ消費量
3. 5 GB に近づいたら以下のいずれかを実施

#### 対処オプション

| 案 | 内容 | コスト影響 |
|---|---|---|
| **A** | 「不要な音声を削除してください」とスタッフに周知 | ¥0 |
| **B** | 古い音声を手動で一括削除（DB の Transcript は残せる） | ¥0 |
| **C** | Pro Plan ($20/月) に昇格、最大 1 TB に拡張 | +$15/月（¥約 2,250） |
| **D** | 自動削除機能を実装（例: 30 日経過した音声を削除） | 開発工数のみ |

#### 推奨運用

- まず A（スタッフ周知）で様子見
- それでも 4 GB を超えたら B（手動削除）
- 「削除運用が回らない」と分かったら D（自動削除実装）
- 規模が拡大して必要なら C（Pro 昇格）
